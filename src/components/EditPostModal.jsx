...
const ProductSchema = Yup.object().shape({
  ...
  content: Yup.mixed().required("Nội dung bài viết không được để trống"),
});

function EditPostModal({ data, setReload, reload }) {
              ...
              onSubmit={(values, actions) => {
                Modal1.confirm({
                  ...
                  onOk: () => {
                    ...
                    dispatch(updatePost({ id: data?.id, data: formData }))
                      .unwrap()
                      .then(() => {
                        setReload(!reload);
                        setOpenModal(false);
